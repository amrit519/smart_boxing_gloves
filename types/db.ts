...
export function initDatabase(): void {
  ...
  // Migrations — safely add any missing columns to existing DB
  const migrations = [
  ...
  for (const sql of migrations) {
    try { db.execSync(sql); } catch { /* column exists, skip */ }
  }
...
export async function saveBatch(rows: RawPacket[], sessionId: string): Promise<void> {
